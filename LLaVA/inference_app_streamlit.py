"""
Streamlit 可视化验证界面：PolarVLM Stage 2（LLaVA 1.5 架构）

功能概览：
- 统一加载 Stage 2 训练好的模型（双流：RGB + Polar + VAE 编码器，基于 LLaVA 1.5）
- 支持多种验证模式：
  1）单张图片（通过路径）验证
  2）上传 RGB + Polar 图片验证
...
# ======================= 基本配置 ======================= #
# 更新为最新的 Stage 2 训练输出目录（当前使用 llava_train_stage2_final/checkpoint-100）
DEFAULT_STAGE2_CKPT = "/openbayes/input/input0/llava_train_stage2_final/checkpoint-100"
DEFAULT_LLM = "/openbayes/input/input0/models/llava-v1.5-13b"
DEFAULT_CLIP = "/openbayes/input/input0/models/clip-vit-large-patch14-336"
...
def load_model_cached(
    ...
    load_4bit: bool = False,  # 🟢 默认关闭 4-bit，使用 FP16 全精度（推荐双卡 4090）
):
    """带缓存的多模态 PolarVLM（Stage 2，LLaVA 1.5 架构）模型加载，避免重复初始化。"""
    # 在加载前清理显存
    ...
    model, tokenizer, image_processor = load_trained_model(
        ...
        hf_token=None,
        device=device,
        load_4bit=load_4bit,  # 🟢 使用用户选择（默认 False，推荐双卡 4090 使用 FP16）
...
# ======================= 页面布局 ======================= #
st.set_page_config(
    page_title="PolarVLM Stage 2 可视化验证（LLaVA 1.5）",
    layout="wide",
)

st.title("PolarVLM Stage 2 可视化验证界面（LLaVA 1.5）")
st.markdown(
    "双流架构（RGB + Polar + VAE 编码器，基于 LLaVA 1.5），用于交互式验证 Stage 2 训练效果。"
)

# 添加模型加载状态说明
with st.expander("ℹ️ 模型加载说明", expanded=False):
    st.markdown("""
    **默认配置（未合并的模型，推荐）**：
    - ✅ `checkpoint_dir`: `/openbayes/input/input0/llava_train_stage2_new/checkpoint-400`（Stage 2 最佳 checkpoint）
    - ✅ `llm_model_name`: `/openbayes/input/input0/models/llava-v1.5-13b`（基础模型）
    - ✅ 自动加载 LoRA 适配器（`adapter_model.safetensors`）
    - ✅ 自动加载更新后的 `polar_projector`（从 `non_lora_trainables.bin`）
    
    **使用合并后的模型（可选）**：
    1. 先运行合并脚本：
       ```bash
       python LLaVA/scripts/merge_lora_weights.py \
           --model-path /openbayes/input/input0/llava_train_stage2_new/checkpoint-400 \
           --model-base /openbayes/input/input0/models/llava-v1.5-13b \
           --save-model-path /openbayes/input/input0/llava_train_stage2_new_merged \
           --polar-vae-model-path /openbayes/input/input0/models/sd-vae-ft-mse
       ```
    2. 在侧边栏修改路径：
       - `checkpoint_dir`: `/openbayes/input/input0/llava_train_stage2_new_merged`
       - `llm_model_name`: `/openbayes/input/input0/llava_train_stage2_new_merged`
...
# 添加模型类型说明
st.sidebar.markdown("**📋 模型加载说明**")
st.sidebar.info(
    "**默认：未合并的模型（推荐）**\n\n"
    "- 使用 Stage 2 checkpoint（包含 LoRA 适配器）\n"
    "- 自动加载 `adapter_model.safetensors` + `non_lora_trainables.bin`\n"
    "- 无需手动合并，直接使用训练输出\n\n"
    "**如需使用合并后的模型：**\n"
    "- 先运行 `merge_lora_weights.py` 合并模型\n"
    "- 将 `checkpoint_dir` 改为合并后的路径（如 `.../stage2_merged`）\n"
    "- 将 `llm_model_name` 也改为合并后的路径"
)

checkpoint_dir = st.sidebar.text_input(
    "Stage 2 checkpoint 路径 (--checkpoint_dir)",
    value=DEFAULT_STAGE2_CKPT,
    help="默认使用未合并的 Stage 2 checkpoint。如果使用合并后的模型，请改为合并后的路径。"
)
llm_model_name = st.sidebar.text_input(
    "LLM 模型路径 (--llm_model_name)",
    value=DEFAULT_LLM,
    help="基础 LLM 模型路径。如果使用合并后的模型，请改为合并后的路径。"
)
...
# 🟢 量化选项（默认关闭，推荐双卡 4090 使用 FP16 全精度）
load_4bit = st.sidebar.checkbox(
    "使用 4-bit 量化（不推荐，精度较低）",
    value=False,
    help="⚠️ 双卡 4090 显存充裕，建议关闭此选项使用 FP16 全精度模式（精度更高，速度更快）"
)
if load_4bit:
    st.sidebar.warning("⚠️ 已启用 4-bit 量化（精度较低，速度较慢）。建议关闭以使用 FP16 全精度模式。")
else:
    st.sidebar.success("🚀 使用 FP16 全精度模式（推荐，精度更高，速度更快）")
...
if device == "cuda" and torch.cuda.is_available():
    num_gpus = torch.cuda.device_count()
    if num_gpus > 1:
        use_multi_gpu = st.sidebar.checkbox(
            f"使用多GPU（检测到 {num_gpus} 个GPU）",
            value=True,
            help="启用后，模型将自动分布到多个GPU上，提升推理速度并降低单卡显存占用"
        )
        if use_multi_gpu:
            if load_4bit:
                st.sidebar.warning(f"⚠️ 4-bit 量化模式下可能无法充分利用多GPU，建议关闭量化")
            else:
                st.sidebar.info(f"✓ 多GPU模式：模型将自动分布到 {num_gpus} 个 GPU（FP16 全精度）")
    else:
        st.sidebar.info(f"检测到 {num_gpus} 个 GPU，将使用单GPU模式")
...
with st.sidebar:
    ...
    if device == "cuda" and torch.cuda.is_available():
        num_gpus = torch.cuda.device_count()
        if st.button("清理显存", help="清理所有 GPU 的显存缓存"):
            for i in range(num_gpus):
                torch.cuda.set_device(i)
                torch.cuda.empty_cache()
                torch.cuda.synchronize()
            st.success(f"✓ 已清理 {num_gpus} 个 GPU 的显存")
        
        # 显示所有GPU的显存使用情况
        st.markdown("**GPU 显存使用情况：**")
        for i in range(num_gpus):
            torch.cuda.set_device(i)
    ...
    if st.button("加载 / 重新加载模型", type="primary"):
        ...
        if device == "cuda" and torch.cuda.is_available():
            ...
            for i in range(num_gpus):
                ...
                torch.cuda.empty_cache()
                torch.cuda.synchronize()
            st.info(f"正在清理显存并加载模型...（{'多GPU' if use_multi_gpu and num_gpus > 1 else '单GPU'}模式）")

# 尝试加载模型，如果失败则显示友好的错误信息
try:
    model, tokenizer, image_processor = load_model_cached(
        checkpoint_dir=checkpoint_dir,
        llm_model_name=llm_model_name,
        clip_model_name=clip_model_name,
        vae_model_path=vae_model_path,
        device=device,
        use_multi_gpu=use_multi_gpu,
        load_4bit=load_4bit,  # 🟢 传递量化选项（默认 False，推荐双卡 4090 使用 FP16）
    )
except torch.cuda.OutOfMemoryError as e:
    st.error("❌ CUDA 显存不足，无法加载模型")
    st.markdown("""
    **解决方案：**
    
    1. **清理显存**：点击侧边栏的「清理显存」按钮
    2. **重启应用**：重启 Streamlit 应用（这会释放所有显存）
    3. **检查其他进程**：运行 `nvidia-smi` 查看是否有其他进程占用显存
    4. **使用 CPU 模式**：如果显存持续不足，可以切换到 CPU 模式（较慢但不会显存不足）
    ...
    **当前显存状态：**
    ...
    if torch.cuda.is_available():
        ...
        st.metric("显存使用", f"{allocated:.2f} / {total:.2f} GB", f"已保留: {reserved:.2f} GB")
    st.stop()
except Exception as e:
    ...
    **可能的解决方案：**
    ...
    1. 检查模型路径是否正确
    2. 确认 checkpoint 目录包含必要的文件（adapter_model.safetensors, non_lora_trainables.bin 等）
    3. 查看终端/日志获取详细错误信息
    """)
    st.stop()
        ...
        if st.button("运行验证", type="primary"):
            ...
            if HAS_CANVAS and bbox_norm == [0.1, 0.1, 0.5, 0.5] and qa_mode == "使用预设模板问题（多种类型）":
                ...
                with st.spinner("正在预处理图像并生成回答..."):
                    ...
                    st.info(f"📊 RGB 图像原始尺寸: {original_size[0]}x{original_size[1]} (预处理后会 resize 到 336x336)")
                    ...
                    st.success(f"✓ RGB 特征尺寸: {rgb_shape} (应为 (1, 3, 336, 336) - LLaVA 1.5)")
    ...
    if st.button("运行验证（单张图片）", type="primary"):
        ...
        if missing:
            ...
            with st.spinner("正在预处理图像并生成回答..."):
                ...
                rgb_pil_check = Image.open(rgb_path).convert("RGB")
                original_size = rgb_pil_check.size
                st.info(f"📊 RGB 图像原始尺寸: {original_size[0]}x{original_size[1]} (预处理后会 resize 到 336x336 - LLaVA 1.5)")
