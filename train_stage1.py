...
# 这对于长时间训练非常重要，可以实时监控训练进度
if sys.stdout.isatty():
    # 如果是终端，保持默认行为
    pass
else:
    # 如果不是终端（如 nohup 重定向），禁用缓冲
...
def create_mae_model(
    ...
    # ⚠️ 临时禁用 torch.load 安全检查（用于加载预训练模型）
    # 这是因为 transformers 库的 from_pretrained 内部使用 torch.load
    # 如果遇到安全限制错误，可以设置此环境变量
    import os
    original_env = os.environ.get("TRANSFORMERS_OFFLINE", None)
    # 尝试设置环境变量来避免 torch.load 安全检查
    # 注意：这只影响 transformers 库内部的 torch.load 调用
    try:
        # 设置环境变量，让 transformers 优先使用 safetensors
    ...
    try:
        ...
        # 直接加载3通道预训练权重
        # 优先使用 safetensors 格式以避免 torch.load 安全限制
        try:
            ...
            print("✓ 已加载3通道预训练权重（使用 safetensors 格式）")
        except Exception as e:
            # 如果 safetensors 加载失败，尝试普通格式
            print(f"⚠ 警告: safetensors 加载失败: {e}")
            print("  尝试使用普通格式加载...")
            try:
                model = ViTMAEForPreTraining.from_pretrained(
                    model_name,
                ...
                print("✓ 已加载3通道预训练权重（使用普通格式）")
            except Exception as e2:
                print(f"❌ 错误: 无法加载预训练权重: {e2}")
                print("  提示: 如果遇到 torch.load 安全限制，请:")
                print("    1. 升级 torch 到 >= 2.6")
                print("    2. 或使用 safetensors 格式的模型")
                print("    3. 或手动下载模型权重文件")
                raise
...
def main(
    ...
    gradient_accumulation_steps: int = 1,  # 梯度累积步数（有效批次大小 = per_device_train_batch_size * gradient_accumulation_steps）
    ...
    # 计算总步数
    # 注意：实际训练步数 = len(train_loader) / gradient_accumulation_steps * num_train_epochs
    ...
    print(f"  - 有效批次大小: {effective_batch_size} (= {per_device_train_batch_size} × {gradient_accumulation_steps})")
    ...
    training_args = TrainingArguments(
        ...
        logging_first_step=True,  # 记录第一步的日志（确保实时输出）
        ...
        # 注意：metric_for_best_model 应该使用 "eval_loss"（带前缀）
        # Trainer 会自动在验证metrics中查找带 "eval_" 前缀的键
        metric_for_best_model="eval_loss" if load_best_model_at_end else metric_for_best_model,  # 用于选择最佳模型的指标
    ...
    class MAETrainer(Trainer):
        ...
        def evaluation_loop(self, dataloader, description, prediction_loss_only=None, ignore_keys=None, metric_key_prefix="eval"):
            ...
            # ========== 4. 手动构造 metrics ==========
            # 关键修复：不要添加 "loss" 键，只添加带前缀的键
            # 这样可以避免 Trainer 误用验证loss覆盖训练loss的显示
            metrics = {
            ...
            if metric_key_prefix == "eval":
                ...
                # 注意：metric_for_best_model 设置为 "loss"，但 Trainer 会自动查找 "eval_loss"
                # 如果 metric_for_best_model="loss"，Trainer 会查找 "eval_loss" 或 "loss"
                # 为了兼容，我们只在需要时添加 "loss" 键（但这里不添加，避免覆盖训练loss）
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gradient_accumulation_steps", type=int, default=1,
                        help="梯度累积步数（有效批次大小 = per_device_train_batch_size × gradient_accumulation_steps）")
